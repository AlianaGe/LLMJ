...
def check_res(
    ...
    if gt_label == pred_label_exchange:
        correct[1] = 1
    if pred_label_exchange == pred_label and pred_label != -1:
        agree = 1
...
if __name__ == "__main__":
    ...
    for idx, (sourcedata, preddata) in enumerate(zip(source, pred)):
        ...
        if pred_label == -1 or pred_label_exchange == -1:
            illformed += 1
            continue
    ...
    for k, v in results.items():
        if k == "Overall":
            print("---------------------------------------------------------")
        print(f"{k:<20}\t{v['agreement']:<10}\t{v['consistency']:<10}")

    print("illformed rate:{:.2f}".format(illformed / len(source)))
